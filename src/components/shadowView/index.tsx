...
function ShadowInner(props: ShadowProps): JSX.Element {
  ...
  const [childLayoutWidth, setChildLayoutWidth] = useState<
    number | undefined
  >();
  const [childLayoutHeight, setChildLayoutHeight] = useState<
    number | undefined
  >();
  ...
  const cRadii: Record<Corner, number | undefined> = useMemo(() => {
    return {
      topStart:
        cStyle.borderTopStartRadius ??
        cStyle.borderTopLeftRadius ??
        cStyle.borderRadius,
      topEnd:
        cStyle.borderTopEndRadius ??
        cStyle.borderTopRightRadius ??
        cStyle.borderRadius,
      bottomStart:
        cStyle.borderBottomStartRadius ??
        cStyle.borderBottomLeftRadius ??
        cStyle.borderRadius,
      bottomEnd:
        cStyle.borderBottomEndRadius ??
        cStyle.borderBottomRightRadius ??
        cStyle.borderRadius,
    };
  }, [cStyle]);
  ...
  const {
    style,
    sRadii,
  }: {style: ViewStyle; sRadii: Record<Corner, number | undefined>} =
    useMemo(() => {
      const style = styleStr
        ? StyleSheet.flatten<ViewStyle>(JSON.parse(styleStr))
        : {};
...
function sanitizeRadii({
  ...
  let radiiSanitized = objFromKeys(cornersArray, k =>
    R(Math.max(radii[k] ?? 0, 0))
  );
  ...
  if (typeof width === 'number' && typeof height === 'number') {
    ...
    const minRatio = Math.min(
      ...
      divDps(
        width,
        sumDps(radiiSanitized.bottomStart, radiiSanitized.bottomEnd)
      ),
      divDps(
        height,
        sumDps(radiiSanitized.topStart, radiiSanitized.bottomStart)
      )
    );
...
function getShadow({
  ...
  const widthWithAdditional =
    typeof width === 'string' ? width : width + additional;
  /** Will (+ additional), only if its value isn't '100%'. [*4] */
  const heightWithAdditional =
    typeof height === 'string' ? height : height + additional;
  ...
  const {topStartShadow, topEndShadow, bottomStartShadow, bottomEndShadow} =
    cornerShadowRadius;
      ...
      {activeCorners.bottomEnd && bottomEndShadow > 0 && (
        <Svg
          ...
          style={{
            ...
            start: width,
            transform: [
              {translateX: isRTL ? bottomEnd : -bottomEnd},
              {translateY: -bottomEnd},
            ],
          }}>
          <Defs>
            {radialGradient2({
...
function getResult({
  ...
  setChildLayoutHeight: React.Dispatch<
